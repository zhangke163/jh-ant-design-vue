				  		...
				  		<a-col v-for="i in 10" :key="i" :span="6" :style="{ display: i < count ? 'block' : 'none' }">
				  			...
				  			<a-form-item :label="`任务小类`" v-if="i===3">
				  				<a-select v-decorator="[
				  				 ...
				  				 placeholder="任务小类" :disabled="showDisabled" @change="handleSelectDetialChange">
				  			...
				  			<a-form-item :label="`细分类型`" v-if="i===4">
				  				<a-select v-decorator="[
				  				 ...
				  				 placeholder="细分类型" :disabled="showDisabled1">
...
<script>
	...
	import { getlocalStorageItm } from "@/utils/common";
	export default{
		data(){
			return {
				 expand: false,
					...
					sbuser:1,
					sbdepart:'单位',
					showDisabled:true  ,//任务小类disabled
					showDisabled1:true  //细分类型disabled
			} 
		},
		components: {
		...
		mounted(){
			var userInfo = getlocalStorageItm("userInfo");
			this.sbuser = userInfo.realName;
			this.sbdepart = userInfo.department;
		},
		computed: {
			count () {
